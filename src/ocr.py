...
class ExtractorSignature(dspy.Signature):
    ...
    pdf_image: dspy.Image = dspy.InputField(desc="Image with text as optical characters")
...
class GeminiExtractor(Extractor):
    ...
    def __init__(self,model:str="gemini-2.5-flash-preview-04-17",temperature:float=0.1):
    ...
    def run(self,image:bytes,prompt:str=None):
        ...
        text = prompt or "Extract the text from the image. Reply directly. DO NOT TRANSLATE!"
...
class DspyExtractor(Extractor):
    ...
    def __init__(self,
        ...
        if model.startswith("ollama_chat"):
            ...
            if api_key is None: 
                raise ValueError("Set environment variable 'GOOGLE_API_KEY' for Gemini model.")
            lm = dspy.LM(model, 
                         temperature=temperature,
                         cache=cache,
                         api_key=api_key,
    ...
    def run(self,image:bytes,prompt:str=None):
        ...
        assert isinstance(image, bytes), f"Expected type 'bytes' but found {type(image)}"
